		...
		/*
		 ...
		 3. 출현횟수
		 	...
		 	3.2 ?
		 		...
		 		ex) 홍?길동 > 안녕 ~ 길동아~ 네 이름은 홍길동이지? > 홍길동, 길동
		 		ex) 홍길동? > 안녕 ~ 길동아~ 네 이름은 홍길동이지? > 홍길동 만약 홍길이 있으면 홍길을 찾음
		 		ex) (자바)?코드 > 자바코드가 있습니다. 그런데 코드에 에러가 있어요. 추가로 자코드가 있습니다.
		 	3.3 +
		 		- 바로 앞의 대상의 출현 횟수가 생략X, 중복O
		 		- 출현 횟수 : 1 ~ 무한대
		 		ex) a+bc > abc > ab, abc, abb, ac, bc, bbc, aabc > abc, aabc
		 		ex) 홍+길동 > 안녕 ~ 길동아~ 네 이름은 홍길동이지? 홍홍홍홍홍길동 > 홍길동, 홍홍홍홍홍길동
		 		ex) (자바)+코드 > 자바코드가 있습니다. 그런데 코드에 에러가 있어요. 자바자바자바코드 추가로 자코드가 있습니다. > 자바코드 자바자바코드
		 	3.4 *
		 	...
		 	4.선택, Choice
		 	 ...
		 	 - []안의 문자 중 1개를 검색
		 		4.1 [열거값]
		 			...
		 			ex)영어 소문자 >[abcdrfghijklmnopqrstuvwxyz]+
		 			ex)영어 대문자 >[ABCDEFGHOJKLMNOPQRSTUVWXYZabcdrfghijklmnopqrstuvwxyz]+
		 		...
		 		5. 출연횟수
		 			...
		 			5.1 {n}
		 				- n : 출현 횟수
		 				ex) a{1}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				ex) a{2}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				ex) a{3}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 			5.2 {n,m}
		 				- n : 최소 출현 횟수
		 				- m : 최대 출현 횟수
		 				ex) a{1,2}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				ex) a{2,3}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				ex) a{0,1}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 			5.3 {n,}
		 				- n : 최소 출련 횟수
		 				- 생략 : 무한대
		 				ex) a{1,}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				ex) a{0,}bc > ab, abc, abb, ac, bc, abbc, bbc, aabc, aabc
		 				
		 				ex) a?bc
		 				ex) a{0,1}bc
		 				
		 			ex) 주민등록번호 > [0-9]{6}-[0-9]{7} //현역
		 			ex) 전화번호 > 010-[0-9]{3,4}-[0-9]{4}
		 			
		 			ex) 한글 2~4자 > [가-힣]{2,4}
		 			ex) 영문자 5자 이상 > [A-Za-z]{5, }
		 			ex) 영문자 5자 이상 + 첫문자가 대문자로 시작 > [A-Z][A-Za-z]{4, }
...
//		txt.replaceAll("정규표현식", "바뀔문자열")
		...
		System.out.println(txt
				.replaceFirst("[0-9]{3}-[0-9]{3,4}-[0-9]{4}", "xxx-xxxx-xxxx"));
