...
//			}
	...
	private static void m9() {
		...
		// 요구사항] 일정관리 > 날짜별 폴더 생성 > [2024-01-18], [2024-01-01]....
		// 2024년 365일 전부~
...
//			//폴더 없음
//			
//			boolean result = dir.mkdir(); //make directoty
...
//		}
		...
		if (!dir.exists()) {
			...
			boolean result = dir.mkdirs(); // mkdirs > 중간폴더까지 생성 mkdir > 중간폴더를 만들어야하면 false
			System.out.println(result);
	...
	private static void m5() {
		// 이동
		// 수정이랑 메소드가 같음 renameTo
		// 경로와 파일의 이름을 동시에 수정할 수 있다. file\score.txt >> move\jumsu.txt
		String path = "C:\\class\\code\\java\\file\\score.txt";
		File file = new File(path);
...
//		System.out.println(file.exists());
		if (file.exists()) {
			...
			System.out.println(file.isFile()); // true 파일인지 폴더인지 확인 isDirectory(폴더)
			...
			System.out.println(file.lastModified()); // 1705547417615 마지막으로 수정한 날짜 > 에폭타임
			// thick > 년월일시분초
			...
			System.out.println(file.canRead()); // true 읽기 가능 여부
			System.out.println(file.canWrite()); // true 쓰기 가능 여부
			System.out.println(file.getParent()); // C:\class\code\java\file 폴더위치
